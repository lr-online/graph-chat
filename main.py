...
class KnowledgeGraph:
    ...
    def add_knowledge(self, concepts: List[Dict[str, str]], context: str = "") -> List[str]:
        """添加知识到图谱中，返回新添加的节点列表"""
        new_nodes = []
        for concept in concepts:
            ...
            if concept_name not in self.graph:
                ...
                new_nodes.append(f"新概念: {concept_name} ({concept['type']}) - {concept['description']}")
            else:
                # 更新现有概念
                update_log = self.update_concept(concept_name, {
                    "type": concept["type"],
                    "description": concept["description"]
                })
        ...
        try:
            ...
            for msg in self.messages:
                if msg.embedding is not None:
                    similarity = self._calculate_similarity(query_embedding, msg.embedding)
...
class Agent:
    ...
    def reply(self, user_input: str) -> Tuple[str, Dict]:
        """处理用户输入并生成回复"""
        # 记录用户消息
        user_message = self.memory.add_message(user_input, "user")
        
        # 获取相关上下文
        relevant_history = self.memory.get_relevant_history(user_input, top_k=3)
        relevant_context = "\n".join([
...
请基于这些上下文来回答用户的问题。如果历史信息或相关概念对回答有帮助，可以参考它们，但不要直接重复这些内容。"""
        ...
        assistant_message = self.memory.add_message(assistant_content, "assistant")

        # 返回回复内容和调试信息
        debug_info = {
            "user_message": {
                "topic": user_message.topic,
                "importance": user_message.importance
            },
            "assistant_message": {
                "topic": assistant_message.topic,
                "importance": assistant_message.importance
            },
            "relevant_concepts": list(related_concepts)
        }

        return assistant_content, debug_info
...
if __name__ == "__main__":
    ...
    while True:
        ...
        print(f"\n助手: {response}")
        print(f"\n[Debug] 消息分析:")
        print(f"用户消息 - 主题: {debug_info['user_message']['topic']}, 重要性: {debug_info['user_message']['importance']}")
        print(f"助手消息 - 主题: {debug_info['assistant_message']['topic']}, 重要性: {debug_info['assistant_message']['importance']}")
